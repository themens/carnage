...
<p align="center">
  <a href="https://themens.github.io/carnage/pages/Getting%20Started/creating-client.html">Getting Started</a> •
  <a href="https://themens.github.io/carnage/pages/Getting%20Started/basic-functions.html">Basic Function</a> •
...
Documentation at https://themens.github.io/carnage/index.html
...
```javascript
// Supports ES6
// import { create, Whatsapp } from 'carnage-bot';
const carnage = require('carnage-bot');
...
function start(client) {
  client.onMessage((message) => {
    if (message.body === 'Hi' && message.isGroupMsg === false) {
      client
        .sendText(message.from, 'Welcome carnage 🕷')
        .then((result) => {
...
##### After executing `create()` function, **carnage** will create an instance of whatsapp web. If you are not logged in, it will print a QR code in the terminal. Scan it with your phone and you are ready to go!

##### carnage will remember the session so there is no need to authenticate everytime.

##### Multiples sessions can be created at the same time by pasing a session name to `create()` function:

```javascript
// Init sales whatsapp bot
carnage.create('sales').then((salesClient) => {...});

// Init support whatsapp bot
carnage.create('support').then((supportClient) => {...});
```
...
carnage `create()` method third parameter can have the following optional parameters:

If you are using the `Linux` server do not forget to pass the args `--user-agent`
[Original parameters in browserArgs](https://github.com/themens/carnage/blob/master/src/config/puppeteer.config.ts)
...
carnage
  .create(
    ...
    // options
    {
      ...
      mkdirFolderToken: '', //folder directory tokens, just inside the carnage folder, example:  { mkdirFolderToken: '/node_modules', } //will save the tokens folder in the node_modules directory
      ...
      autoClose: 60000, // Automatically closes the carnage-bot only when scanning the QR code (default 60 seconds, if you want to turn it off, assign 0 or false)
...
| `chatsAvailable`       | When carnage is connected to the chat list |
...
// Send file (carnage will take care of mime types, just need the path)
...
await client.setProfileName('carnage bot');
...
// Event interface is in here: https://github.com/s2click/carnage/blob/master/src/api/model/participant-event.ts
...
There are some tricks for a better usage of carnage.
...
Building carnage is really simple altough it contains 3 main projects inside
